...
const AddNewAddressView = ({
  ...
  // console.log('New add',newAddress.value);
  return (
...
const Checkout = () => {
  ...
  // console.log('checkout',newAddress.value);
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      let balance=window.localStorage.getItem("balance")-getTotalCartValue(items);
  ...
  useEffect(() => {
    ...
    const onLoadHandler = async () => {
      ...
      // console.log(productsData,cartData,addressData)
        ...
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            {addresses?addresses.all.map((address)=>{
              return(<>
                <Box id={address._id} onClick={(e)=>handleBoxClick(e.target.id)} className={addresses.selected===address._id?"address-item selected":"address-item not-selected nn"}>{address.address}
                <Button onClick={()=>{deleteAddress(token,address._id)}} startIcon={<DeleteIcon />}>
  Delete
</Button>
</Box>
</> )
